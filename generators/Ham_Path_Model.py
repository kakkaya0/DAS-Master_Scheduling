...
class S_TSP:
    def __init__(self, network, relevant_edges, relevant_nodes,origin,destination):
    ...
    def calculate_dist_dict(self):
        ...
        # self.dist = {edge: self.network.edges[list(edge)[0], list(edge)[-1]]['cost'] for edge in self.edges}
    ...
    def build_model(self):
        ...
        self.degree_constr = {
            ...
            for n in self.nodes if n not in self.origin and n not in self.destination}
        
        self.degree_origin_constr = {
            origin: self.m.addConstr(gp.quicksum(self.x[edge] for edge in self.edges if origin in edge) == 1,
                           name=f'service_flow_{origin}')
            for origin in self.origin}
        
        self.degree_destination_constr = {
            destination: self.m.addConstr(gp.quicksum(self.x[edge] for edge in self.edges if destination in edge) == 1,
                           name=f'service_flow_{destination}')
