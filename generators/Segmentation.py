...
# Create a new dictionary to calculate the service times and upper bounds for each sample
 ...
 for i in range(number_of_samples):
    # Get the service time values for each sample and calculate the Probability Mass Function(PMF) and Cumulative Distribution Function (CDF)
      service_time_values = [all_samples_list[i][result][value_names[1]] for result in all_samples_list[i]]
      val, cnt = np.unique(service_time_values, return_counts=True)
      pmf = cnt / len(service_time_values)
    ...
    # Calculate mean of travel time values to find actual departure time 
      ...
      upper_bound = np.interp(given_probability, cdf, val)
...
def DAS_line_master_scheduling_fixed(instance,speed,number_of_samples,dimension_of_samples,standard_deviation,epsilon,time_window_width):
...
# Set the lower and upper bound to 0 for the first compulsory stop
 ...
 # Calculate pmf and cdf of Dh (route duration)       
             val_service, cnt_service = np.unique(service_time_values, return_counts=True)   
             pmf_service = cnt_service / sum(cnt_service)
             cdf_service = np.cumsum(pmf_service)

 # Calculate pmf and cdf of Lh-1 (departure time from the previous compulsory stop)         
             val_departure, cnt_departure = np.unique(departure_time_previous_values, return_counts=True)   
             pmf_departure = cnt_departure / sum(cnt_departure)
             cdf_departure = np.cumsum(pmf_departure)

# Calculate pmf and cdf of Th as sum of dh and lh-1 and find the 1-epsilon-percentile value to make sure that we are serving every requested optional stop with that probability
             th_values = service_time_values + departure_time_previous_values
             val_th, cnt_th = np.unique(th_values, return_counts=True)  
             pmf_th = cnt_th/sum(cnt_th)
 ...
 #Set the upper bound(bh) to the calculated Th value and store upper bound values of each dimension in a list          
             upper_bound = np.interp(given_probability, cdf_th, val_th)
...
# Calculate the actual departure times (same as above)
             for sample_index in range(number_of_samples):
              ...
              for dimension_index in range(dimension_of_samples):
                Lh_previous = all_segments_list[segment_index-1][sample_index][dimension_index]['Departure Time']
  ...
  # If the calculated sd is not smaller than our input threshold, then we increase the dimension size by 10 and re-sample          
              ...
              return  DAS_line_master_scheduling_fixed(instance,speed,number_of_samples,dimension_of_samples,standard_deviation,epsilon,time_window_width)
...
def DAS_line_master_scheduling_non_fixed(instance,speed,number_of_samples,dimension_of_samples,standard_deviation,epsilon):
...
# Iterate through each segment to calculate departure times and upper bound values for each sample
    for segment_index in range(len(segments)):
        upper_bound_values = list() # Create a list to store the upper bound values for each sample
        lower_bound_values = list() # Create a list to store the lower bound values of each sample
...
# Set the lower and upper bound to 0 for the first compulsory stop
 ...
 # Calculate pmf and cdf of Lh-1 (departure time from preivous compulsory stop)         
             val_departure, cnt_departure = np.unique(departure_time_previous_values, return_counts=True)   
             pmf_departure = cnt_departure / sum(cnt_departure)
             cdf_departure = np.cumsum(pmf_departure)
...
# Calculate pmf and cdf of Th as sum of Dh and Lh-1 and find the 1-epsilon-percentile value to make sure that we are serving every requested optional stop with that probability
 ...
 #Set the upper bound(bh) to the calculated Th value and store upper bound values of each dimension in a list          
             upper_bound = np.interp(given_probability, cdf_th, val_th)
             upper_bound_values.append(upper_bound)
             given_probability_upper = 1-epsilon
             given_probability_lower = epsilon

 #Set the upper bound(bh) to the calculated Th value and store upper bound values of each dimension in a list          
             upper_bound = np.interp(given_probability_upper, cdf_th,val_th)
             upper_bound_values.append(upper_bound)

 # Calculate and store the lower bound values
             lower_bound = np.interp(given_probability_lower, cdf_th, val_th)
             lower_bound_values.append(lower_bound)

#Calculate the mean and standard deviation of upper and lower bound values for that sample
            upper_bound_mean = np.mean(upper_bound_values)
            upper_bound_sd = np.std(upper_bound_values)
            lower_bound_mean = np.mean(lower_bound_values)
            lower_bound_sd = np.std(lower_bound_values)


# If calculated standard deviation is smaller than our input sd, then we continue as our solution is precise
# Store the calculated mean value as the upper bound and the upper bound of the previous compulsory stops as lower and upper bounds of the current stop
            if upper_bound_sd < standard_deviation and lower_bound_sd < standard_deviation:
             lower_upper_bound[segment_index] = {
                value_names[0]: lower_bound_mean,
                value_names[1]: upper_bound_mean
             }
...
# Calculate the actual departure times (same as above)
  ...
  # If the calculated sd is not smaller than our input threshold, then we increase the dimension size by 10 and re-sample          
              ...
              dimension_of_samples = dimension_of_samples + 10
              return  DAS_line_master_scheduling_non_fixed(instance,speed,number_of_samples,dimension_of_samples,standard_deviation,epsilon)
...
# the final method running the fixed or non_fixed method based on the width_type_input
def DLMSP (width_type,instance,speed,number_of_samples,dimension_of_samples,standard_deviation,epsilon,time_window_width):

    if width_type == "fixed":
         return DAS_line_master_scheduling_fixed(instance,speed,number_of_samples,dimension_of_samples,standard_deviation,epsilon,time_window_width)

    elif width_type == "adjustable":
         return DAS_line_master_scheduling_non_fixed(instance,speed,number_of_samples,dimension_of_samples,standard_deviation,epsilon)
